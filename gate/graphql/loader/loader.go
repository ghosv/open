...
const (
	appLoaderKey   key = "app"
	userLoaderKey  key = "user"
	orgLoaderKey   key = "org"
	groupLoaderKey key = "group"
)
...
func Initialize(service *client.MicroClient) Collection {
	return Collection{
		lookup: map[key]dataloader.BatchFunc{
			appLoaderKey:   newAppLoader(service),
			userLoaderKey:  newUserLoader(service),
			orgLoaderKey:   neworgLoader(service),
			groupLoaderKey: newgroupLoader(service),
		},
	}
}
